...
const googleDriveRoutes = require('./backend/routes/googleDrive');
const {
    uploadFileToDrive,
    TOKEN_PATH: GOOGLE_DRIVE_TOKEN_PATH
} = require('./backend/services/googleDrive');
...
const autoDriveBackupStatePath = path.join(
    userDataPath,
    'google-drive-auto-backup-state.json'
);
...
function loadAutoDriveBackupState() {
    try {
        if (!fs.existsSync(autoDriveBackupStatePath)) return {};
        return JSON.parse(fs.readFileSync(autoDriveBackupStatePath, 'utf8'));
    } catch (e) {
        return {};
    }
}

function saveAutoDriveBackupState(state) {
    try {
        fs.writeFileSync(autoDriveBackupStatePath, JSON.stringify(state, null, 2), 'utf8');
    } catch (e) {
        console.warn('Falha ao salvar estado do backup automático:', e.message);
    }
}

async function uploadDatabaseBackupToGoogleDrive(c) {
    ...
    try {
        ...
        throw new Error('Não foi possível copiar o banco de dados: ' + e.message);
    }
...
async function runDailyDriveBackupOnStartup() {
    const todayKey = getTodayBackupKey();
    const state = loadAutoDriveBackupState();
    ...
    try {
        const tokenFlowAvailable = fs.existsSync(GOOGLE_DRIVE_TOKEN_PATH);
        let uploaded = null;
        let mode = null;

        if (tokenFlowAvailable) {
            const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
            const tmpName = `database-backup-auto-${Date.now()}.db`;
            const tmpPath = path.join(userDataPath, tmpName);
            try {
                fs.copyFileSync(dbPath, tmpPath);
                uploaded = await uploadFileToDrive(
                    tmpPath,
                    `negocar-database-auto-${stamp}.db`,
            ...
            uploaded = await uploadDatabaseBackupToGoogleDrive(c);
            mode = 'legacy_refresh_token';
        }

        saveAutoDriveBackupState({
            lastBackupDate: todayKey,
            lastBackupAt: new Date().toISOString(),
            lastBackupFileId: uploaded.id || null,
            lastBackupFileName: uploaded.name || null,
            mode
        });
        console.log('[Backup Diário] Backup automático enviado para Google Drive:', uploaded.name || uploaded.id);
    } catch (err) {
        console.error('[Backup Diário] Falha ao executar backup automático:', err && err.message ? err.message : err);
    }
}

// --- Backup Local Manual + Automático Diário ---

const localBackupConfigPath = path.join(userDataPath, 'local-backup-config.json');

function loadLocalBackupConfig() {
    try {
        if (!fs.existsSync(localBackupConfigPath)) {
            return {
                pastaBackup: '',
        ...
        return JSON.parse(fs.readFileSync(localBackupConfigPath, 'utf8'));
    } catch (e) {
        return {
            pastaBackup: '',
            ultimoBackupAutomatico: ''
...
function saveLocalBackupConfig(config) {
    fs.writeFileSync(localBackupConfigPath, JSON.stringify(config, null, 2), 'utf8');
}

function getTodayLocalBackupKey() {
    const now = new Date();
...
function gerarNomeBackupLocal(tipo = 'manual') {
    const now = new Date();
    ...
    return `backup-negocar-${tipo}-${ano}-${mes}-${dia}-${hora}-${minuto}-${segundo}.db`;
}

function fazerBackupLocal(tipo = 'manual') {
    const config = loadLocalBackupConfig();

    if (!config.pastaBackup) {
        return {
            success: false,
            message: 'Nenhuma pasta de backup foi configurada.'
        };
    }
    ...
    if (!fs.existsSync(config.pastaBackup)) {
        fs.mkdirSync(config.pastaBackup, { recursive: true });
    }

    const nomeArquivo = gerarNomeBackupLocal(tipo);
    const destino = path.join(config.pastaBackup, nomeArquivo);
...
// Variável para rastrear notificação de backup automático na sessão
let lastAutoBackupNotification = null;

function runDailyLocalBackupOnStartup() {
    try {
        const config = loadLocalBackupConfig();

        if (!config.pastaBackup) {
            console.log('[Backup Local] Nenhuma pasta configurada. Backup automático ignorado.');
            return;
        }

        const hoje = getTodayLocalBackupKey();

        if (config.ultimoBackupAutomatico === hoje) {
            console.log('[Backup Local] Backup automático já realizado hoje.');
            return;
        }

        const resultado = fazerBackupLocal('automatico');

        if (resultado.success) {
            saveLocalBackupConfig({
                ...config,
                ultimoBackupAutomatico: hoje,
                ultimoBackupEm: new Date().toISOString(),
                ultimoArquivo: resultado.arquivo
            });

            // Armazenar para notificação no frontend
            lastAutoBackupNotification = {
                success: true,
            ...
            console.log('[Backup Local] Backup automático criado:', resultado.arquivo);
        } else {
            console.log('[Backup Local]', resultado.message);
        }

    } catch (error) {
        console.error('[Backup Local] Erro no backup automático:', error.message);
    }
}

app.get('/api/backup-local/config', (req, res) => {
    res.json({
        success: true,
        config: loadLocalBackupConfig()
    });
});

app.post('/api/backup-local/config', (req, res) => {
    const { pastaBackup } = req.body;

    if (!pastaBackup || !String(pastaBackup).trim()) {
        return res.status(400).json({
            success: false,
            message: 'Informe uma pasta válida para backup.'
        });
    }

    const configAtual = loadLocalBackupConfig();

    const novaConfig = {
        ...configAtual,
        pastaBackup: String(pastaBackup).trim()
    };

    saveLocalBackupConfig(novaConfig);

    res.json({
        success: true,
        message: 'Pasta de backup salva com sucesso.',
        config: novaConfig
    });
});

app.post('/api/backup-local/manual', (req, res) => {
    try {
        const resultado = fazerBackupLocal('manual');
        res.status(resultado.success ? 200 : 400).json(resultado);
    } catch (error) {
        res.status(500).json({
            success: false,
            message: 'Erro ao fazer backup local: ' + error.message
        });
    }
});

// Endpoint para verificar notificação de backup automático (retorna uma vez e marca como notificado)
app.get('/api/backup-local/notification', (req, res) => {
    if (lastAutoBackupNotification && !lastAutoBackupNotification.notificado) {
        lastAutoBackupNotification.notificado = true;
        res.json({
...
app.post('/api/backup/drive', async (req, res) => {
    ...
    try {
        const uploaded = await uploadDatabaseBackupToGoogleDrive(c);
        res.json({
            success: true,
